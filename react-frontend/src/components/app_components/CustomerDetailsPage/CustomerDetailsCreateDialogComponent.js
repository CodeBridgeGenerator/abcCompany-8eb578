...
const CustomerDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,address: _entity?.address,contactNo: _entity?.contactNo,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Customer Details created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Customer Details" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Customer Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
