...
import salesDetailsFakerFactory from "./salesDetailsFakerFactory";
...
const SalesDetailsFakerDialogComponent = (props) => {
...
const [productIds, setproductIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("customerDetails")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcustomerIds(data.data);
                        });
client.service("productDetails")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = salesDetailsFakerFactory(props.user,fakerCount,customerIds,productIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("salesDetails").create(elem));
...
export default connect(mapState, mapDispatch)(SalesDetailsFakerDialogComponent);
