...
const SingleSalesDetailsPage = (props) => {
...
const [product, setProduct] = useState([]);
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleSalesDetailsId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
...
const product = Array.isArray(res.product)
        ...
        setProduct(product);
            ...
            .catch((error) => {
                console.log({ error });
                props.alert({ title: "SalesDetails", type: "error", message: error.message || "Failed get salesDetails" });
            });
    }, [props,urlParams.singleSalesDetailsId]);


    const goBack = () => {
        navigate("/app/salesDetails");
    };
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            <div className="col-12">
                ...
                {/* <p>salesDetails/{urlParams.singleSalesDetailsId}</p> */}
            </div>
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Product</label>
                    {product.map((elem) => (
                        <Link key={elem._id} to={`/productDetails/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.name}</p>
                            </div>
      ...
      <CommentsSection
        recordId={urlParams.singleSalesDetailsId}
        user={props.user}
        alert={props.alert}
...
export default connect(mapState, mapDispatch)(SingleSalesDetailsPage);
