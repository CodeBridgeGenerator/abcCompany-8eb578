...
const SalesDetailsEditDialogComponent = (props) => {
...
price: _entity?.price,
        ...
        await client.service("salesDetails").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info salesDetails updated successfully" });
...
const productOptions = product.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Edit Sales Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="salesDetails-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(SalesDetailsEditDialogComponent);
